...
namespace For_The_Potato_Backend.Services
{
    public class VelemenyService : IVelemeny
    {
        ...
        public VelemenyService(ForThePotatoContext context, ResponseDto responseDto)
        {
            _context = context;
        ...
        public async Task<object> DeleteVelemeny(string userName)
        {
            try
            {
                var user = await _context.Aspnetusers.FirstOrDefaultAsync(s => s.NormalizedUserName == userName.ToUpper());

                if (user != null)
                {
                    var record = await _context.Velemenies.FirstOrDefaultAsync(v => v.Id == user.Id);
        ...
        public async Task<object> GetAll()
        {
            try
            {
                var velemenies = _context.Velemenies;
                _responseDto.Message = "Sikeres lekérés";
                _responseDto.Value = velemenies;
                return _responseDto;
            }
            catch (Exception ex)
            {
        ...
        public async Task<object> GetAVelemeny(string userName)
        {
            try
            {
                ...
                if (user != null) 
                {
                    var velemeny = await _context.Velemenies.FirstOrDefaultAsync(v => v.Id == user.Id);
        ...
        public async Task<object> PostVelemeny(VelemenyDto velemenyDto)
        {
            try
            {
                if (velemenyDto != null)
                {
                    ...
                    var user = await _context.Aspnetusers.FirstOrDefaultAsync(u => u.NormalizedUserName == velemenyDto.UserName.ToUpper() && u.Velemeny == null);
        ...
        public async Task<object> UpdateVelemeny(VelemenyDto velemenyDto)
        {
            try
            {
                if (velemenyDto != null)
                {
                    var user = await _context.Aspnetusers.FirstOrDefaultAsync(u => u.NormalizedUserName == velemenyDto.UserName.ToUpper());
                    ...
                    if (user != null)
                    {
                        ...
                        if (velemeny != null)
                        {
                            ...
                            _context.Velemenies.Update(velemeny);
                            await _context.SaveChangesAsync();
